% 用分支定界法求解最优路径
function [best_path, best_cost] = branchbound(N, n, dist_spot, dist_repo)
    % n是linehaul的个数
    % N是节点总的个数
    ...
    while isempty(pq)~=1
        if hn.size == n  && option ~= 0   % linehaul的最后一层，接下来就要在backhaul上选择了
            new_impose = n+1:N;
            for j = 1:length(new_impose)
                rcost = hn.remaincost - min_out(hn.num);
                cc = hn.currentcost + dist_spot(hn.num, new_impose(j));
                ...
                if b < best_c
                    ...
                    node.path = [hn.path, new_impose(j)];
                    pq = [pq, node];
                end                
            end
            pq = rank(pq);
